...
namespace IntervalTree
{
	...
	/// </summary>
	/// <typeparam name="T"></typeparam>
	public class IntervalTree<T, TValue> where T : IComparable<T>
	{
		#region Fields
		...
		private readonly KeyValueComparer<T, TValue> keyvalueComparer;
		...
		public IntervalTree(IEnumerable<KeyValuePair<Interval<T>, TValue>>? elems)
		{
			if (elems != null)
			{
			...
			this.keyvalueComparer = new KeyValueComparer<T, TValue>(this.comparer);
		}
		...
		/// </summary>
		/// <param name="arg">The arg.</param>
		public bool Add(Interval<T> interval, TValue value)
		{
			bool wasAdded = false;
		...
		public void GetIntervalsOverlappingWith(Interval<T> toFind, ref List<KeyValuePair<Interval<T>, TValue>> list)
		{
			if (this.Root != null)
			{
		...
		/// <param name="toFind">To find.</param>
		/// <returns></returns>
		public IEnumerable<KeyValuePair<Interval<T>, TValue>>? GetIntervalsOverlappingWith(Interval<T> toFind)
		{
			return Root?.GetIntervalsOverlappingWith(toFind);
		}
		...
		/// <param name="arg">The arg.</param>
		/// <returns></returns>
		public List<KeyValuePair<Interval<T>, TValue>> GetIntervalsStartingAt(T arg)
		{
		...
		/// Those intervals starting at the same time/value are sorted further based on their End value (i.e. returned in ascending order of their End values)
		/// </summary>
		public IEnumerable<KeyValuePair<Interval<T>, TValue>> IntervalValuePairs
		{
			get
			{
				if (this.Root == null)
				...
				while (p != null)
				{
					...
					yield return new KeyValuePair<Interval<T>, TValue>(p.Interval, p.Value);
					p = p.Successor();
				}
			}
		}
		...
		/// <returns></returns>
		public bool TryGetInterval(Interval<T> data, out TValue value)
		{
		...
		/// <param name="data">The data.</param>
		/// <returns></returns>
		private bool TryGetIntervalImpl(IntervalNode? subtree, Interval<T> data, out TValue value)
		{
			if (subtree != null)
			{
				...
				else
				{
					...
					{
						int kthIndex = subtree.Range.BinarySearch(new KeyValuePair<T, TValue>(data.End, default), keyvalueComparer);
						if (kthIndex >= 0)
						{
		...
		private class IntervalNode
		{
			...
			public List<KeyValuePair<T, TValue>>? Range { get; private set; }
			...
			public IntervalNode(Interval<T> interval, TValue value)
			{
				this.Left = null;
				this.Right = null;
				this.Balance = 0;
			...
			/// <param name="data">The data.</param>
			/// <returns></returns>
			public static IntervalNode Add(IntervalNode elem, Interval<T> interval, TValue value, ref bool wasAdded, ref bool wasSuccessful)
			{
				if (elem == null)
				{
			...
			/// </summary>
			/// <returns></returns>
			public IEnumerable<KeyValuePair<Interval<T>, TValue>> GetRange()
			{
				if (this.Range != null)
				{
					foreach (var value in this.Range)
					{
						...
						yield return new KeyValuePair<Interval<T>, TValue>(kth, value.Value);
					}
				}
				else
				{
			...
			/// </summary>
			/// <returns></returns>
			public IEnumerable<KeyValuePair<Interval<T>, TValue>> GetRangeReverse()
			{
				if (this.Range != null && this.Range.Count > 0)
				{
					...
					for (int k = rangeCount - 1; k >= 0; k--)
					{
						...
						yield return new KeyValuePair<Interval<T>, TValue>(kth, this.Range[k].Value);
					}
				}
				else
				{
			...
			/// <param name="data">The data.</param>
			/// <returns></returns>
			public static List<KeyValuePair<Interval<T>, TValue>>? GetIntervalsStartingAt(IntervalNode? subtree, T start)
			{
				if (subtree != null)
				{
					...
					{
						var result = new List<KeyValuePair<Interval<T>, TValue>>();
						...
						result.Add(new KeyValuePair<Interval<T>, TValue>(subtree.Interval, subtree.Value));
						return result;
					}
				}
				else
			...
			public void GetIntervalsOverlappingWith(Interval<T> toFind, ref List<KeyValuePair<Interval<T>, TValue>> list)
			{
				...
				else
				{
					...
					{
						list ??= new List<KeyValuePair<Interval<T>, TValue>>();

						list.Add(new KeyValuePair<Interval<T>, TValue>(this.Interval, this.Value));
			...
			public IEnumerable<KeyValuePair<Interval<T>, TValue>> GetIntervalsOverlappingWith(Interval<T> toFind)
			{
				...
				else
				{
					...
					if (this.Interval.OverlapsWith(toFind))
					{
						yield return new KeyValuePair<Interval<T>, TValue>(this.Interval, this.Value);
			...
			private bool DeleteIntervalFromNodeWithRange(Interval<T> interval)
			{
				if (this.Range != null && this.Range.Count > 0)
				{
					...
					{
						var keyvalueComparer = new KeyValueComparer<T, TValue>(ComparerUtil.GetComparer());
						int k = Range.BinarySearch(new KeyValuePair<T, TValue>(interval.End, default), keyvalueComparer);
						if (k >= 0)
						{
			...
			private void AddIntervalValuePair(Interval<T> interval, TValue value)
			{
				this.Range ??= new List<KeyValuePair<T, TValue>>();

				////always store the max End value in the node.Data itself .. store the Range list in decreasing order
				if (interval.End.CompareTo(this.Interval.End) > 0)
				{
					this.Range.Insert(0, new KeyValuePair<T, TValue>(this.Interval.End, this.Value));
					this.Interval = interval;
					this.Value = value;
				}
				else
				{
					...
					for (int i = 0; i < this.Range.Count; i++)
					{
						...
						{
							this.Range.Insert(i, new KeyValuePair<T, TValue>(interval.End, value));
							wasAdded = true;
							break;
						}
					}
					if (!wasAdded)
					{
						this.Range.Add(new KeyValuePair<T, TValue>(interval.End, value));
					}
				}
			}
		...
		private class KeyValueComparer<TKey, TValue> : IComparer<KeyValuePair<TKey, TValue>>
		{
			...
			{
				return obj is KeyValueComparer<TKey, TValue> comparer && Equals(keyComparer, comparer.keyComparer);
			}
		...
		public static class ComparerUtil
		{
			...
			{
				return typeof(IComparable<T>).IsAssignableFrom(typeof(T)) || typeof(IComparable).IsAssignableFrom(typeof(T))
